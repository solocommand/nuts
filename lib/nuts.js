...
function Nuts(opts) {
    ...
    this.router.get('/feed', (req, res) => {
        res.redirect('/feed/channel/all.atom');
    })
...
Nuts.prototype.onServeVersionsFeed = function(req, res, next) {
    ...
    .then(function(versions) {
        _.each(versions, function(version) {
            const baseLink = urljoin(fullUrl, '/../../../', '/download/version/'+version.tag);
            const files = [
                { platform: 'windows_64', format: 'exe' },
                { platform: 'windows_64', format: 'msi' },
                { platform: 'windows_64', format: 'zip' },
                { platform: 'osx', format: 'dmg' },
                { platform: 'osx', format: 'zip' },
            ].map(({ platform, format }) => {
                const link = urljoin(baseLink, `/${platform}?filetype=${format}`);
                feed.addItem({
                    title: version.tag,
                    link,
                    // files,
